...
def digest_to_html(digest_response) -> str:
    from app.agents.digest_agent import EmailDigestResponse
...
def send_email_to_self(subject: str, body_text: str, body_html: str):
    if not MY_EMAIL:
        raise ValueError("MY_EMAIL environment variable is not set. Please set it in your .env file.")
    send_email(subject, body_text, body_html, recipients=[MY_EMAIL])
