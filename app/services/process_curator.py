...
def curate_digests(hours: int = 24) -> dict:
    ...
    logger.info(f"Curating {total} digests from last {hours} hours")
    ...
    for article in ranked_articles[:10]:
        digest = next((d for d in digests if d["id"] == article.digest_str), None)
        if digest:
            logger.info(f"\nRank {article.rank} | Score: {article.relevance_score: 1.f}/10.0")
        ...
        "articles": [
            {
                ...
                "rank": a.rank,
                "relevence_score": a.relevance_score,
